...
PositionController::PositionController() : Node("control") {
    twtpub_ = this->create_publisher<geometry_msgs::msg::Twist>("/cmd_vel", 10);

    using std::placeholders::_1;
    odomsub_ = this->create_subscription<nav_msgs::msg::Odometry>("/odom", 10, std::bind(&PositionController::loop, this, _1));
    goalsub_ = this->create_subscription<geometry_msgs::msg::PoseStamped>("/goal_pose", 10, std::bind(&PositionController::get_goal, this, _1));
}

double PositionController::compute_yaw(double qw, double qx, double qy, double qz) {
    return atan2(2.0f * (qw * qz + qx * qy), qw * qw + qx * qx - qy * qy - qz * qz);
}

void PositionController::compute_control(double x, double y, double yaw, double &vc, double &wc) {
...
void PositionController::get_goal(const geometry_msgs::msg::PoseStamped::SharedPtr msg) {
    xref = msg->pose.position.x;
    yref = msg->pose.position.y;
    yawref = PositionController::compute_yaw(msg->pose.orientation.w, msg->pose.orientation.x, msg->pose.orientation.y, msg->pose.orientation.z);
}
...
void PositionController::loop(const nav_msgs::msg::Odometry::SharedPtr msg) {
    double yaw = PositionController::compute_yaw(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z);

    // compute control velocity
    PositionController::compute_control(msg->pose.pose.position.x, msg->pose.pose.position.y, yaw, vc, wc);

    // limit
    PositionController::velocity_limit(vc, wc);
